...
namespace Ardita.Areas.Report.Controllers
{
    [CustomAuthorize]
    [Area("Report")]
    public class ReportArchiveLoansInActiveController : BaseController<ArchiveLoanParams>
    {
        ...
        {
            var reportName = "RptArchiveLoan";
            var returnString = await _reportService.GenerateReportArchiveLoansInActive(reportName, param, AppUsers.CurrentUser(User));
        ...
        public async Task AllViewBag()
        {
            ...
            ViewBag.ListBorrowers = await BindBorrower();
            ViewBag.ListPics = await BindEmployee();
        }
    }
}
