...
namespace Ardita.Repositories.Classess
{
    public class ArchiveExtendDetailRepository : IArchiveExtendDetailRepository
    {
        ...
        {
            IEnumerable<TrxArchiveExtendDetail> result;

            var propertyInfo = typeof(TrxArchiveExtendDetail).GetProperty(model.sortColumn, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
            var propertyName = propertyInfo == null ? typeof(TrxArchiveExtendDetail).GetProperties()[0].Name : propertyInfo.Name;

            if (model.sortColumnDirection.ToLower() == "asc")
            {
                result = await _context.TrxArchiveExtendDetails
                    //.Include(x => x.Status)
                    ...
                    .OrderBy(x => EF.Property<TrxArchiveExtendDetail>(x, propertyName))
            ...
            else
            {
                result = await _context.TrxArchiveExtendDetails
                    ...
                    //)
                    //.Contains(model.searchValue))
                    .OrderByDescending(x => EF.Property<TrxArchiveExtendDetail>(x, propertyName))
                    .Skip(model.skip).Take(model.pageSize)
                    .ToListAsync();
            }
