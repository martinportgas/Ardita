...
namespace Ardita.Repositories.Classess
{
    public class ArchiveDestroyDetailRepository : IArchiveDestroyDetailRepository
    {
        ...
        public async Task<IEnumerable<TrxArchiveDestroyDetail>> GetAll(string par = " 1=1 ")
        {
            var results = await _context.TrxArchiveDestroyDetails
                .Include(x => x.ArchiveDestroy)
                .Where(x => x.IsActive == true && x.ArchiveDestroy.IsActive == true && x.ArchiveDestroy.StatusId != 4)
                .Where(par)
                .ToListAsync();
            return results;
        }
